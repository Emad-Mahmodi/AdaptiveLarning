...
if strcmp(task_type, 'bc'),
    switch upper(method)
        ...
        case {'PA1','PA2','PA1_C','PA2_C'}
            ...
            return;
        case {'ALMA','ALMA_C'}
            options.eta = 0.9500%0.9;  % alpha(eta) \in (0,1]
            options.p   = 10;%2;
            options.C   = 0.0625;%sqrt(2);
            return;
        case {'OGD','OGD_C'}
            ...
            return;
        case {'CW','CW_C'}
            options.eta = 0.8500;%0.7;  % in \eta in [0.5,1]
            options.a   = 1;
            return;
        case {'AROW','AROW_C'}
            options.C = 16;%1;      % i.e., parameter r
            options.a = 1;      % default
            return;
        case {'SOP','SOP_C'}
            options.a = 1;
        ...
        case {'IELLIP','IELLIP_C'}
            options.b = 0.6000;%0.3;
            options.IELLIP_c = 0.1000;%0.1; % c=0.5
            options.a = 1;
            return;
        case {'SCW','SCW_C'}
            options.eta = 0.9000;%0.75;
            options.C   = 0.1250;%1;
            options.a   = 1;
            return;
        case {'SCW2','SCW2_C'}
            options.eta = 0.8500;%0.90;
            options.C   = 0.5000;%1;
            options.a   = 1;
            return;
        case {'NAROW','NAROW_C'}
            options.C = 0.0625;%1; %i.e., parameter r
            options.NAROW_b = 1;
            options.a = 1;
            return;
        case {'NHERD','NHERD_C'}
            options.C = 0.1250;%1; % NHERD_C = C
            options.a = 1;
            return;
